...
model = ResNet50(weights=None, include_top=False, input_shape=(320,240,3))
...
for i in range(EPOCH):
    for count in range(int(len(x_train)/batch)):
        ...
        for img in x_train[count*batch: (count+1)*batch]:
            ...
            images.append(image)
            Count+=1
        images = np.array(images).reshape(batch,320,240,3)
        out = np.array(y_train[count*batch: (count+1)*batch]).reshape(batch,4)
        out[:,0],out[:,2] = out[:,0]*(320/640),out[:,2]*(320/640)
        out[:,1],out[:,3] = out[:,1]*(240/480),out[:,3]*(240/480)
        print('Training the model')
        model.fit(images,out,epochs=2,batch_size=50)
